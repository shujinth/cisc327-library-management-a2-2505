...
def return_book_by_patron(patron_id: str, book_id: int) -> Tuple[bool, str]:
    ...
    success_return = update_borrow_record_return_date(patron_id, book_id, datetime.now())
    success_update = update_book_availability(book_id, +1)

    if not success_return or not success_update:
        return False, "Database error while processing return."

    # Calculate late fee (business logic function from R5)
    late_fee_data = calculate_late_fee_for_book(patron_id, book_id)
    fee = late_fee_data.get("fee_amount", 0.0)

    if fee > 0:
        return True, f'Book successfully returned. Late fee: ${fee:.2f}'
    else:
        return True, "Book successfully returned. No late fees."
...
def calculate_late_fee_for_book(patron_id: str, book_id: int) -> Dict:
    ...
    for record in borrowed_books:
        if record["book_id"] == book_id:
            ...
            # First 7 days: $0.50/day, after that $1/day, max $15
            fee = 0.0
            if days_overdue <= 7:
    ...
    return {"fee_amount": 0.00, "days_overdue": 0, "status": "Book not borrowed or already returned"}
...
def get_patron_status_report(patron_id: str) -> Dict:
    ...
    for record in borrowed_books:
        ...
        late_data = calculate_late_fee_for_book(patron_id, record["book_id"])
        total_fees += late_data["fee_amount"]
        ...
        report_books.append({
            ...
            "fee": late_data["fee_amount"]
        })

    return {
        "patron_id": patron_id,
        "currently_borrowed": report_books,
        "borrow_count": total_books,
        "total_late_fees": round(total_fees, 2),
        "status": "Active" if total_books > 0 else "No borrowed books"
    }
